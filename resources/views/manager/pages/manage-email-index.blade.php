                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="">
                            ...
                            <a href="{{ route('manager.manage-email.create') }}" class="btn btn-outline-primary"><i
                                    ...
                                    @forelse ($email as $key => $item)
                                        <tr>
                                            ...
                                            <td class="d-flex justify-content-center align-items-center">
                                                <a href="{{ route('manager.manage-email.view', $item->contact_code) }}" style="margin-right: 10px" class="btn btn-outline-primary">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <form id="delete-form-{{ $item->id }}"
                                                    action="{{ route('manager.manage-email.destroy', $item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a type="button" class="bs-tooltip btn btn-rounded btn-outline-danger"
                                                        ...
                                                        data-url="{{ route('manager.manage-email.destroy', $item->id) }}"
                                                        data-name="{{ $item->name }}"
