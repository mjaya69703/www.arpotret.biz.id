...
@section('content')
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="">
                            ...
                            <a href="{{ route('manager.manage-blog.posts-create') }}" class="btn btn-outline-primary"><i class="fa-solid fa-plus"></i></a>
                                    ...
                                    @foreach ($post as $key => $item)
                                        <tr>
                                            ...
                                            <td class="d-flex justify-content-center align-items-center">
                                                <a href="{{ route('manager.manage-blog.posts-edit', $item->id) }}" style="margin-right: 10px" class="btn btn-outline-primary">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form id="delete-form-{{ $item->id }}"
                                                    action="{{ route('manager.manage-blog.posts-destroy', $item->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a type="button" class="bs-tooltip btn btn-rounded btn-outline-danger"
                                                        data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                        data-original-title="Delete"
                                                        data-url="{{ route('manager.manage-blog.posts-destroy', $item->id) }}"
                                                        data-name="{{ $item->name }}"
    ...
    <div class="me-1 mb-1 d-inline-block">
        ...
        <form action="{{ route('manager.manage-blog.category-update', $item->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
