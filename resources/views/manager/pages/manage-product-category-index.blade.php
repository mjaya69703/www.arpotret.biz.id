    ...
    <section class="section">
        <div class="row">
            <div class="col-lg-4 col-12 mb-3">
                <form action="{{ route('manager.manage-product.category.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                                        ...
                                        <tr>
                                            ...
                                            <td class="d-flex justify-content-center align-items-center">
                                                ...
                                                <form id="delete-form-{{ $item->id }}"
                                                    action="{{ route('manager.manage-product.category.destroy', $item->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a type="button" class="bs-tooltip btn btn-rounded btn-outline-danger"
                                                        data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                        data-original-title="Delete"
                                                        data-url="{{ route('manager.manage-product.category.destroy', $item->id) }}"
                                                        data-name="{{ $item->name }}"
    ...
    @foreach ($pCategory as $item)
        <div class="modal fade text-left w-100" id="xUpdateCategory{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel16" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                <form action="{{ route('manager.manage-product.category.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
