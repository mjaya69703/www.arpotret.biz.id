                        ...
                        @foreach ($gallery as $item)
                            <div class="col-lg-4 col-md-6 portfolio-item filter-{{ $item->cproduct->slug }}">
                                <div class="portfolio-content h-100">
                                    <img src="{{ asset('storage/images/gallery/cover/' . $item->gallery_cover) }}"
                                        class="img-fluid" alt="">
                                    <div class="portfolio-info">
                                        <h4>{{ $item->cproduct->name }}</h4>
                                        <p>{{ $item->gallery_name }}</p>
                                        <a href="{{ asset('storage/images/gallery/cover/' . $item->gallery_cover) }}"
                                            title="{{ $item->gallery_name }}" data-gallery="portfolio-gallery-remodeling"
                                            class="glightbox preview-link"><i class="bi bi-zoom-in"></i></a>
                                        <a href="{{ route('root.root-main-portofolio-detail', $item->gallery_slug) }}"
